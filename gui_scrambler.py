...
class ScramblerGUI:
    ...
    def _setup_styles(self):
        ...
        bg_color = "#f0e6ff"  # Light lavender
        accent_color = "#ffb3ba"  # Light pink
        text_color = "#6b5b95"  # Dark lavender
        button_color = "#bae1ff"  # Light blue
        slider_color = "#baffc9"  # Light green
        
        self.style.configure("TFrame", background=bg_color)
        self.style.configure("TLabel", background=bg_color, foreground=text_color, font=("Arial", 10))
        self.style.configure("TButton", background=button_color, foreground=text_color, font=("Arial", 10, "bold"))
        self.style.map("TButton", background=[("active", "#a1d6ff")])
        
        self.style.configure("Header.TLabel", font=("Arial", 24, "bold"), foreground=text_color)
        self.style.configure("Subheader.TLabel", font=("Arial", 14), foreground=text_color)
        
        self.style.configure("TCheckbutton", background=bg_color, foreground=text_color, font=("Arial", 12))
        self.style.map("TCheckbutton", background=[("active", accent_color)])
        
        self.style.configure("TScale", background=bg_color, troughcolor=slider_color, sliderlength=30)
        self.style.configure("Value.TLabel", font=("Arial", 10), foreground=text_color)
        
        self.style.configure("Status.TLabel", font=("Arial", 14, "bold"), padding=15, background=accent_color)
        self.style.configure("Active.TLabel", foreground="#4caf50", background=accent_color)
        self.style.configure("Inactive.TLabel", foreground="#f44336", background=accent_color)
        self.style.configure("Error.TLabel", foreground="#f44336", background=accent_color)
    ...
    def _create_settings(self):
        ...
        try:
            ...
            # Base delay slider
            ttk.Label(settings_frame, text="Base Delay:", style="Subheader.TLabel").pack()
            ...
            self.delay_slider.pack(fill='x', pady=10)
        except Exception as e:
    ...
    def _create_status(self):
        ...
        try:
            ...
            self.status_indicator.pack(pady=30, padx=20, fill='x')
        except Exception as e:
    ...
    def _animate_toggle(self, enabled):
        ...
        def _animate(alpha):
            if enabled:
                color = f"#{int(186 * alpha):02x}{int(225 * alpha):02x}{int(255 * alpha):02x}"
            else:
                color = f"#{int(255 * (1-alpha)):02x}{int(179 * (1-alpha)):02x}{int(186 * (1-alpha)):02x}"
